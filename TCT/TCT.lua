...
local TCTFrame = CreateFrame("Frame", "TCTFrame", UIParent)
TCTFrame.point = {"BOTTOM", UIParent, -207, 200} --Позиция иконок (сразу все 4 после /reload)
local TCTPlayerFrame = CreateFrame("Frame", "TCTPlayerFrame",TCTFrame)
...
local TCTParty1Frame = CreateFrame("Frame", "TCTParty1Frame",TCTFrame)
...
local TCTParty2Frame = CreateFrame("Frame", "TCTParty2Frame",TCTFrame)
...
local TCTArena1Frame = CreateFrame("Frame", "TCTArena1Frame",TCTFrame)
    TCTArena1Frame:SetPoint("CENTER", "UIParent", 200, 50)
    TCTArena1Frame:SetSize(150,ArenaFrameSize)
    TCTArena1Frame:SetBackdropColor(0,0,0,.5)
    TCTArena1Frame.elements = {}

local TCTArena2Frame = CreateFrame("Frame", "TCTArena2Frame",TCTFrame)
    TCTArena2Frame:SetPoint("CENTER", "UIParent", 200, 0)
    TCTArena2Frame:SetSize(150,ArenaFrameSize)
    TCTArena2Frame:SetBackdropColor(0,0,0,.5)
    TCTArena2Frame.elements = {}
    
local TCTArena3Frame = CreateFrame("Frame", "TCTArena3Frame",TCTFrame)
    TCTArena3Frame:SetPoint("CENTER", "UIParent", 200, -50)
...
if not TCTFrame.eventFrame then
    ...
    TCTFrame.eventFrame:RegisterEvent("PARTY_MEMBERS_CHANGED")
...
function TCTFrame:COMBAT_LOG_EVENT_UNFILTERED(frame, timestamp, event, sourceGUID, sourceName, sourceFlags, destGUID, destName, destFlags, spellID, spellName)
--Populate GUIDs again
...
function TCTFrame:CreateElements()
    --Player Frame
    for i=11, 14 do
      local TCTPlayerIconFrame = CreateFrame("frame", "TCTPlayerIconFrame "..i,TCTPlayerFrame)
        TCTPlayerIconFrame.cd2 = CreateFrame("cooldown", "PlayerCDFrame "..i, TCTPlayerIconFrame)
    ...
    --Party1
    for i=11, 14 do
      local TCTParty1IconFrame = CreateFrame("frame", "TCTParty1IconFrame "..i,TCTParty1Frame)
        TCTParty1IconFrame.cd2 = CreateFrame("cooldown", "TCTParty1CDFrame "..i, TCTParty1IconFrame)
    ...
    --Party2
    for i=11, 14 do
      local TCTParty2IconFrame = CreateFrame("frame", "TCTParty2IconFrame "..i,TCTParty2Frame)
        TCTParty2IconFrame.cd2 = CreateFrame("cooldown", "TCTParty2CDFrame "..i, TCTParty2IconFrame)
    ...
    --Arena1
    for i=11, 14 do
      local TCTArena1IconFrame = CreateFrame("frame", "TCTArena1IconFrame "..i,TCTArena1Frame)
        TCTArena1IconFrame.cd2 = CreateFrame("cooldown", "TCTArena1CDFrame "..i, TCTArena1IconFrame)
    ...
    --Arena2
    for i=11, 14 do
      local TCTArena2IconFrame = CreateFrame("frame", "TCTArena2IconFrame "..i,TCTArena2Frame)
        TCTArena2IconFrame.cd2 = CreateFrame("cooldown", "TCTArena2CDFrame "..i, TCTArena2IconFrame)
    ...
    --Arena3
    for i=11, 14 do
      local TCTArena3IconFrame = CreateFrame("frame", "TCTArena3IconFrame "..i,TCTArena3Frame)
        TCTArena3IconFrame.cd2 = CreateFrame("cooldown", "TCTArena3CDFrame "..i, TCTArena3IconFrame)
...
function TCTFrame:PARTY_MEMBERS_CHANGED()
        ...
        for i=11, 14 do
            ...
            if itemID==id then
                if(duration>0) then 
                    ...
                    TCTPlayerFrame.elements[i].cd2:SetCooldown(GetTime, duration - 0.5)
                    TCTPlayerFrame.elements[i].cd2:SetScript("OnUpdate", function(fr)   
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
        ...
        for i=11, 14 do
            ...
            if itemID==id then
                if(duration>0) then 
                    ...
                    TCTParty1Frame.elements[i].cd2:SetCooldown(GetTime, duration - 0.5)
                    TCTParty1Frame.elements[i].cd2:SetScript("OnUpdate", function(fr)                   
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
                            fr:GetParent():SetBackdropBorderColor(.8,.2,.2)
                            fr:SetScript("OnUpdate", nil)
        ...
        for i=11, 14 do
            ...
            if itemID==id then
                if(duration>0) then 
                    ...
                    TCTParty2Frame.elements[i].cd2:SetCooldown(GetTime, duration - 0.5)
                    TCTParty2Frame.elements[i].cd2:SetScript("OnUpdate", function(fr)           
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
                            fr:GetParent():SetBackdropBorderColor(.8,.2,.2)
                            fr:SetScript("OnUpdate", nil)
        ...
        if not contains(rings, itemID) then 
            ...
            if itemID==id then
                ...
                if(duration>0) then 
                    ...
                    TCTArena1Frame.elements[i].cd2:SetScript("OnUpdate", function(fr)               
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
                            fr:GetParent():SetBackdropBorderColor(.8,.2,.2)
                            fr:SetScript("OnUpdate", nil)
        ...
        if not contains(rings, itemID) then 
            ...
            if itemID==id then
                ...
                if(duration>0) then 
                    ...
                    TCTArena2Frame.elements[i].cd2:SetScript("OnUpdate", function(fr)               
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
                            fr:GetParent():SetBackdropBorderColor(.8,.2,.2)
                            fr:SetScript("OnUpdate", nil)
        ...
        if not contains(rings, itemID) then 
            ...
            if itemID==id then
                ...
                if(duration>0) then 
                    ...
                    TCTArena3Frame.elements[i].cd2:SetScript("OnUpdate", function(fr)               
                        if not select(6,UnitBuff(unit, buffname)) then
                            fr:SetReverse(false)
                            fr:GetParent():SetBackdropBorderColor(.8,.2,.2)
                            fr:SetScript("OnUpdate", nil)
